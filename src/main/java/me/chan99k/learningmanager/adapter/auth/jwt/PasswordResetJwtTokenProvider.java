package me.chan99k.learningmanager.adapter.auth.jwt;

import java.time.Instant;
import java.time.temporal.ChronoUnit;
import java.util.Date;

import javax.crypto.SecretKey;

import org.springframework.beans.factory.annotation.Value;
import org.springframework.stereotype.Component;

import io.jsonwebtoken.Claims;
import io.jsonwebtoken.Jwts;
import io.jsonwebtoken.security.Keys;
import me.chan99k.learningmanager.adapter.auth.AuthProblemCode;
import me.chan99k.learningmanager.adapter.auth.PasswordResetTokenProvider;
import me.chan99k.learningmanager.adapter.auth.TokenRevocationProvider;
import me.chan99k.learningmanager.common.exception.AuthException;
import me.chan99k.learningmanager.domain.member.Email;

@Component
public class PasswordResetJwtTokenProvider extends AbstractJwtTokenProvider<SecretKey>
	implements PasswordResetTokenProvider {
	private static final String TOKEN_PURPOSE = "PASSWORD_RESET";

	private final String issuer;
	private final String audience;
	private final SecretKey secretKey;
	private final long validityInMilliseconds;

	public PasswordResetJwtTokenProvider(
		TokenRevocationProvider<SecretKey> revocationProvider,
		@Value("${jwt.password-reset-token.issuer}") String issuer,
		@Value("${jwt.password-reset-token.audience}") String audience,
		@Value("${jwt.secret}") String secret,
		@Value("${jwt.password-reset-token.validity-in-seconds}") long validityInSeconds
	) {
		super(revocationProvider);
		this.issuer = issuer;
		this.audience = audience;
		this.secretKey = Keys.hmacShaKeyFor(secret.getBytes());
		this.validityInMilliseconds = validityInSeconds * 1000;
	}

	@Override
	public String createResetToken(String email) {
		Instant now = Instant.now();
		Instant expires = now.plus(validityInMilliseconds, ChronoUnit.MILLIS);

		return Jwts.builder()
			.issuer(issuer)
			.subject(email)
			.audience().add(audience).and()
			.issuedAt(Date.from(now))
			.expiration(Date.from(expires))
			.claim("purpose", TOKEN_PURPOSE)
			.signWith(secretKey)
			.compact();
	}

	@Override
	public boolean validateResetToken(String token) {
		try {
			if (isRevoked(token)) {
				return false;
			}
			Claims claims = Jwts.parser()
				.verifyWith(secretKey)
				.requireIssuer(issuer)
				.requireAudience(audience)
				.clockSkewSeconds(60)
				.build()
				.parseSignedClaims(token)
				.getPayload();

			return TOKEN_PURPOSE.equals(claims.get("purpose", String.class));
		} catch (Exception e) {
			throw new AuthException(AuthProblemCode.FAILED_TO_VALIDATE_TOKEN, e);
		}
	}

	@Override
	public Email getEmailFromResetToken(String token) {
		try {
			Claims claims = Jwts.parser()
				.verifyWith(secretKey)
				.build()
				.parseSignedClaims(token)
				.getPayload();

			return Email.of(claims.getSubject());
		} catch (Exception e) {
			throw new AuthException(AuthProblemCode.FAILED_TO_AUTHENTICATE, e);
		}
	}

	@Override
	public void invalidateAfterUse(String token) {
		revokeToken(token);
	}

	@Override
	protected SecretKey getValidator() {
		return this.secretKey;
	}
}
