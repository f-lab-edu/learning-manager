...
- **API 엔드포인트:** `POST /api/v1/auth/token`
- **성공 시나리오:**
    1. 사용자가 로그인 페이지에서 이메일과 비밀번호를 입력하고 '로그인'을 요청한다.
    2. 시스템은 해당 `Account` 정보를 찾아 `Credential`의 비밀번호 일치 여부를 확인한다.
    3. 인증 성공 시, 시스템은 JWT 기반의 Access Token과 Refresh Token을 발급한다.
    4. Refresh Token은 DB에 저장되고, 클라이언트에게 두 토큰이 반환된다.
...
- **실패 시나리오:**
    - **(세션 제약조건 위반):** 생성하려는 세션이 제약조건을 위반할 경우, 관련 오류 메시지를 반환한다.
        - **24시간 초과:** 세션 지속시간이 24시간을 초과하는 경우 "세션은 24시간을 초과할 수 없습니다." 메시지를 반환한다.
        - **시간 검증:** 모든 시간 관련 검증은 Clock을 사용하여 Asia/Seoul 시간대 기준으로 수행된다.
...
- **SessionParticipantRole 정의:**
    - **HOST**: 세션을 주도하고 진행하는 역할 (스터디 매니저, 스터디장 등)
        - 세션 설정 및 출석 관리 권한
        - 세션당 1명만 지정 가능
    - **SPEAKER**: 특정 내용을 발표하거나 강의하는 역할 (발표자, 강연자 등)
        - 발표 자료 공유 및 설명 권한
        - 복수 지정 가능
    - **ATTENDEE**: 일반 참석자 역할 (수강생, 청중 등)
        - 질문 및 토론 참여 권한
        - 기본 역할로 복수 지정 가능
- **성공 시나리오 (추가/제외/역할 변경):**
    1. 스터디장이 세션 참여자 목록에서 다음 중 하나의 작업을 요청한다.
        - **추가:** 신규 참여자의 정보(이메일 등)와 역할(`HOST`, `SPEAKER`, `ATTENDEE`)을 입력하여 추가를 요청한다.
...
- **실패 시나리오:**
    ...
    - **(역할 제약조건 위반):**
        - 세션의 호스트(`HOST`)가 두 명 이상이 되는 경우, "세션의 호스트는 1명만 지정할 수 있습니다." 메시지를 반환한다.
        - 존재하지 않는 역할을 지정하는 경우, "올바르지 않은 참여자 역할입니다." 메시지를 반환한다.
- **연관 도메인:** Member
...
### 3.5 [P0] ✅ 스터디 세션 목록 조회 (Session List Retrieval)
...
### 4.4 [P1] ✅ 출석 수정 요청 (Attendance Correction Request)

- **주요 액터(Actor):** 사용자(스터디 멤버)
- **사전 조건:** 스터디 멤버는 로그인되어 있으며, 수정할 출석 기록이 존재한다.
- **API 엔드포인트:** `POST /api/v1/attendance/{attendanceId}/correction`
- **성공 시나리오:**
    1. 스터디 멤버가 출석 기록 수정을 요청한다.
    2. 시스템은 수정 요청 이벤트를 출석 기록에 추가한다.
    3. 시스템은 관리자에게 검토가 필요함을 알린다.

- **실패 시나리오:**
    - **(권한 없음):** 본인의 출석 기록이 아닌 경우, "권한이 없습니다."라는 메시지를 반환한다.
    - **(이미 수정 요청 중):** 이미 수정 요청이 진행 중인 경우, 오류 메시지를 반환한다.
- **연관 도메인:** Member, Session

### 4.5 [P1] ✅ 출석 수정 승인 (Attendance Correction Approval)

- **주요 액터(Actor):** 사용자(스터디장, 관리자)
- **사전 조건:** 관리자는 로그인되어 있으며, 승인할 수정 요청이 존재한다.
- **API 엔드포인트:** `POST /api/v1/attendance/{attendanceId}/correction/approve`
- **성공 시나리오:**
    1. 관리자가 출석 수정 요청을 검토하고 승인한다.
    2. 시스템은 출석 상태를 요청된 상태로 변경한다.
    3. 시스템은 승인 이벤트를 기록한다.

- **실패 시나리오:**
    - **(권한 없음):** 해당 과정의 관리자가 아닌 경우, "권한이 없습니다."라는 메시지를 반환한다.
- **연관 도메인:** Member, Session

### 4.6 [P1] ✅ 출석 수정 거절 (Attendance Correction Rejection)

- **주요 액터(Actor):** 사용자(스터디장, 관리자)
- **사전 조건:** 관리자는 로그인되어 있으며, 거절할 수정 요청이 존재한다.
- **API 엔드포인트:** `POST /api/v1/attendance/{attendanceId}/correction/reject`
- **성공 시나리오:**
    1. 관리자가 출석 수정 요청을 검토하고 거절한다.
    2. 시스템은 거절 사유와 함께 거절 이벤트를 기록한다.
...
### 5.1 [P0] ✅ 액세스 토큰 갱신 (Refresh Access Token)

- **주요 액터(Actor):** 사용자(회원), 클라이언트 애플리케이션
- **사전 조건:** 사용자는 유효한 Refresh Token을 보유하고 있다.
- **API 엔드포인트:** `POST /api/v1/auth/token/refresh`
- **성공 시나리오:**
    1. 클라이언트가 Access Token 만료를 감지하고, 저장된 Refresh Token으로 갱신을 요청한다.
    2. 시스템은 Refresh Token의 유효성을 검증한다. (만료 여부, 폐기 여부)
    3. 검증 성공 시, 시스템은 새로운 Access Token을 발급하여 반환한다.
    4. (선택) Refresh Token Rotation 정책에 따라 새 Refresh Token도 함께 발급할 수 있다.

- **실패 시나리오:**
    - **(만료된 토큰):** Refresh Token이 만료된 경우, "토큰이 만료되었습니다. 다시 로그인해주세요."라는 메시지를 반환한다.
    - **(폐기된 토큰):** Refresh Token이 폐기된 경우, "유효하지 않은 토큰입니다."라는 메시지를 반환한다.
    - **(존재하지 않는 토큰):** DB에 해당 Refresh Token이 없는 경우, "유효하지 않은 토큰입니다."라는 메시지를 반환한다.
- **연관 도메인:** Member

### 5.2 [P0] ✅ 로그아웃 (Token Revocation)

- **주요 액터(Actor):** 사용자(회원)
- **사전 조건:** 사용자는 로그인된 상태이다.
- **API 엔드포인트:** `POST /api/v1/auth/token/revoke`
- **성공 시나리오:**
    1. 사용자가 '로그아웃'을 요청한다.
    2. 시스템은 해당 사용자의 Refresh Token을 폐기(revoke) 처리한다.
    3. 시스템은 로그아웃 완료를 알린다.
    4. 클라이언트는 저장된 Access Token과 Refresh Token을 삭제한다.

- **실패 시나리오:**
    - **(이미 폐기된 토큰):** 이미 폐기된 토큰으로 요청한 경우에도 성공 응답을 반환한다. (멱등성 보장)
- **연관 도메인:** Member
- **보안 고려사항:**
    - Access Token은 Stateless이므로 즉시 무효화되지 않음
    - 짧은 Access Token TTL 설정으로 보안 리스크 최소화

### 5.3 [P2] ✅ 전체 세션 로그아웃 (Revoke All Tokens)

- **주요 액터(Actor):** 사용자(회원)
- **사전 조건:** 사용자는 로그인된 상태이다.
- **API 엔드포인트:** `POST /api/v1/auth/token/revoke-all`
- **성공 시나리오:**
    1. 사용자가 '모든 기기에서 로그아웃'을 요청한다.
    2. 시스템은 해당 사용자의 모든 Refresh Token을 폐기 처리한다.
    3. 시스템은 완료를 알린다.

- **실패 시나리오:**
    - 특별한 실패 시나리오 없음 (권한 검증은 Access Token으로 수행)
- **연관 도메인:** Member
- **구현 상태:** ✅ 구현 완료
...
### 6.1 [P1] ✅ 시스템 역할 부여 (Grant System Role)

- **주요 액터(Actor):** 사용자(ADMIN, SUPERVISOR)
- **사전 조건:** 관리자는 로그인되어 있으며, ADMIN 또는 SUPERVISOR 역할을 보유하고 있다.
- **API 엔드포인트:** `POST /api/v1/admin/members/{memberId}/roles`
- **성공 시나리오:**
    1. 관리자가 특정 회원에게 시스템 역할 부여를 요청한다.
    2. 시스템은 대상 회원의 존재 여부를 확인한다.
    3. 시스템은 권한 에스컬레이션 여부를 검증한다. (SUPERVISOR는 ADMIN 부여 불가)
    4. 시스템은 역할을 부여하고 201 Created를 반환한다.

- **실패 시나리오:**
    - **(권한 없음):** ADMIN 또는 SUPERVISOR가 아닌 사용자가 요청할 경우, 403 Forbidden을 반환한다.
    - **(권한 에스컬레이션):** SUPERVISOR가 ADMIN 역할을 부여하려는 경우, 오류를 반환한다.
    - **(회원 없음):** 대상 회원이 존재하지 않는 경우, 400 Bad Request를 반환한다.
- **연관 도메인:** Member

### 6.2 [P1] ✅ 시스템 역할 회수 (Revoke System Role)

- **주요 액터(Actor):** 사용자(ADMIN, SUPERVISOR)
- **사전 조건:** 관리자는 로그인되어 있으며, ADMIN 또는 SUPERVISOR 역할을 보유하고 있다.
- **API 엔드포인트:** `DELETE /api/v1/admin/members/{memberId}/roles/{role}`
- **성공 시나리오:**
    1. 관리자가 특정 회원의 시스템 역할 회수를 요청한다.
    2. 시스템은 대상 회원과 역할의 존재 여부를 확인한다.
    3. 시스템은 역할을 회수하고 204 No Content를 반환한다.
...
### 6.3 [P1] ✅ 시스템 역할 조회 (Retrieve System Roles)

- **주요 액터(Actor):** 사용자(ADMIN, SUPERVISOR)
- **사전 조건:** 관리자는 로그인되어 있으며, ADMIN 또는 SUPERVISOR 역할을 보유하고 있다.
- **API 엔드포인트:** `GET /api/v1/admin/members/{memberId}/roles`
- **성공 시나리오:**
    1. 관리자가 특정 회원의 시스템 역할 조회를 요청한다.
    2. 시스템은 대상 회원의 모든 시스템 역할을 반환한다.
...
### 6.4 [P2] ⚠️ 시스템 역할 변경 이력 조회 (Retrieve System Role Audit Log)

- **주요 액터(Actor):** 사용자(ADMIN)
- **사전 조건:** 관리자는 로그인되어 있으며, ADMIN 역할을 보유하고 있다.
- **API 엔드포인트:** `GET /api/v1/admin/audit/system-roles` (향후 구현 예정)
- **성공 시나리오:**
    1. 관리자가 시스템 역할 변경 이력 조회를 요청한다.
    2. 시스템은 필터 조건(기간, 대상 회원, 변경 유형 등)에 따라 변경 이력을 조회한다.
    3. 시스템은 페이징된 변경 이력 목록을 반환한다.
        - 각 이력에는 대상 회원, 변경된 역할, 변경 유형(GRANTED/REVOKED), 수행자, 시각, 사유가 포함된다.

- **실패 시나리오:**
    - **(권한 없음):** ADMIN이 아닌 사용자가 요청할 경우, 403 Forbidden을 반환한다.
- **연관 도메인:** Member
- **구현 상태:**
    - ✅ 감사 로그 저장 (역할 부여/회수 시 자동 기록)
...
### 7.1 [P4] ❌ 알림 발송 (Notification Send)
